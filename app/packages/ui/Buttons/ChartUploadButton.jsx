...
import { UploadChart } from "@/app/packages/supabase/SupabaseBucketImages.js";
...
export default function App({ setNotification }) {
  ...
  return (
    <div className="space-y-2">
      ...
      {!intentUpload && (
        <div>
          <label className="inline-block text-sm font-medium text-primary-accent mt-2.5 mb-1">
            Upload chart
          </label>

          <button className="group p-2 sm:p-4 block cursor-pointer text-center border-2 border-dashed border-gray-200 rounded-lg focus-within:outline-none focus-within:ring-2 focus-within:ring-blue-500 focus-within:ring-offset-2  w-full border border-primary hover:border-[var(--text-primary-accent-secondary)] transition"
            onClick={handleUploadIntent}
          >

            < PlusIcon className="mx-auto icon-color size-100 h-5 w-5" aria-hidden="true" />

            <span className="mt-2 block text-sm text-primary-accent-secondary">
      ...
      {intentUpload && (
        <>
          ...
          <div className="block text-sm font-medium text-primary-accent mt-2">

            Chart image
            {!isChartUploaded && (
              <>
                <label for="af-submit-app-upload-images" className="group p-4 sm:p-7 block cursor-pointer text-center border-2 border-dashed border-gray-200 rounded-lg focus-within:outline-none focus-within:ring-2 focus-within:ring-blue-500 focus-within:ring-offset-2 border border-primary hover:border-[var(--text-primary-accent-secondary)] transition mt-1">
            ...
            {isChartUploaded && (
              <>
                <label for="af-submit-app-upload-images" className="group p-2 sm:p-3 block  text-center border-2 border-dashed border-gray-200 rounded-lg focus-within:outline-none focus-within:ring-2 focus-within:ring-blue-500 focus-within:ring-offset-2 border border-primary hover:border-[var(--text-primary-accent-secondary)] transition mt-1 cursor-default pointer-events-none">
              ...
              <>
                <div className="relative mt-2">
                  ...
                  <div className="relative flex justify-center">
                    <span className="background-color px-2 text-sm text-primary-accent-secondary">Or</span>
                  </div>
                </div>
                <div>
                  <label
                    htmlFor="chartLink"
                    className="block text-sm font-medium text-primary-accent"
                  >
                    Chart link
                  </label>
